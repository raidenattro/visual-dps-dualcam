...
import { apiDelete, apiGet, apiPost, thumbnailUrl } from '../../api/client.js';
import {
...
export function useAnnotateTool(canvasRef, options = {}) {
  ...
  /** 单路 embedded：避免同一缩略图 URL 重复赋值导致 onload 闪屏 */
  const embeddedThumbSrcRef = useRef('');
  const embeddedLoadGenRef = useRef(0);
  ...
  const bootstrapShelfOnFrame = useCallback(
    (frameW, frameH) => {
      const fw = Math.max(1, Number(frameW) || 1);
      const fh = Math.max(1, Number(frameH) || 1);
      if (shelfPointsRef.current.length < 4) {
        shelfPointsRef.current = createDefaultShelfCorners(fw, fh);
        refreshPerspectiveFromShelf();
      }
      const code =
        String(fixedCameraRef.current?.name || fixedCameraRef.current?.id || 'DEFAULT').trim() ||
        'DEFAULT';
      if (embedded && !shelvesRef.current.length) {
        const corners = shelfPointsRef.current.map((p) => [...p]);
        const stub = {
          shelf_code: code,
          shelf_name: code,
          shelf_corners: corners,
          grid_shape: [gridRowsRef.current, gridColsRef.current],
          boxes: [],
        };
        shelvesRef.current = [stub];
        setShelves([stub]);
        activeShelfIndexRef.current = 0;
        setSelectedShelfCode(code);
        setCameraName((prev) => prev || code);
      }
      coordSpaceRef.current = { w: fw, h: fh };
      setCanSave(!!mInvRef.current);
    },
    [embedded, refreshPerspectiveFromShelf, setCameraName],
  );
  ...
  const renderFrameAndEnterAnnotating = useCallback(
    (imageSrc, loadedMsg) => {
      ...
      bgImage.onload = () => {
        ...
        bootstrapShelfOnFrame(bgImage.width, bgImage.height);
        renderAnnotator();
        setStatus(
          loadedMsg ||
            '已生成货架外框（可拖四角）。也可清空后依次点击 4 点；再选行列并「生成货位」。',
          'ok',
        );
        bumpRender();
      };
  ...
  const applyAnnotationPayload = useCallback(
    (payload, options = {}) => {
      ...
      } else if (Array.isArray(ann.shelf_corners) && ann.shelf_corners.length >= 4) {
        const legacyShelfCode =
          (ann.source_info && (ann.source_info.shelf_code || ann.source_info.camera_name)) ||
          String(fixedCameraRef.current?.name || fixedCameraRef.current?.id || 'SHELF_1');
        const shape = Array.isArray(ann.grid_shape) ? ann.grid_shape : [gridRows, gridCols];
        const legacyShelf = {
          shelf_code: legacyShelfCode,
          shelf_corners: ann.shelf_corners,
          grid_shape: shape,
          boxes: Array.isArray(ann.boxes) ? ann.boxes : [],
        };
        setShelves([legacyShelf]);
        activeShelfIndexRef.current = 0;
        setSelectedShelfCode(legacyShelfCode);
  ...
  const loadAnnotationOnStartup = useCallback(
    async (options) => {
      ...
      try {
        ...
        return applyAnnotationPayload(payload, { silent });
      } catch {
        return false;
      }
    },
  ...
  /** embedded 单路：恢复 capture 写入的 JPG，与巷道标注缩略图同源 */
  const loadCameraSavedFrame = useCallback(
    async (options = {}) => {
      const silent = options.silent === true;
      const cam = fixedCameraRef.current;
      ...
      return new Promise((resolve) => {
        ...
        bgImage.onload = () => {
          ...
          annCvs.width = bgImage.width;
          annCvs.height = bgImage.height;
          coordSpaceRef.current = { w: bgImage.width, h: bgImage.height };

          const idx = activeShelfIndexRef.current;
          const shelf =
            idx >= 0 ? shelvesRef.current[idx] : shelvesRef.current[0] || null;
          if (shelf) {
            loadShelfIntoEditor(shelf);
          } else if (shelfPointsRef.current.length < 4) {
            bootstrapShelfOnFrame(bgImage.width, bgImage.height);
          } else {
            refreshPerspectiveFromShelf();
            fillMissingCellPolygons();
          ...
          if (!annotationSizeRef.current?.width || !annotationSizeRef.current?.height) {
            annotationSizeRef.current = {
              width: bgImage.width,
          ...
          setCanSave(!!mInvRef.current);
          if (!silent) {
            setStatus('已加载上次抓帧；更新画面请点「抓帧」。', 'ok');
          }
          resolve(true);
        };
  ...
  const captureFrame = useCallback(async () => {
    ...
    try {
      ...
      if (embedded && camId) {
        data = await apiPost(
          `/api/cameras/${encodeURIComponent(camId)}/capture?height=${GRAB_HEIGHT}`,
          {},
        );
      }
      if (!data?.image) {
      ...
      annotationSourceRef.current = {
        ...
        camera_url: url || fixedCameraRef.current?.url || '',
        camera_name: getCameraNameFromUI(url) || fixedCameraRef.current?.name || '',
        shelf_code: String(getCameraNameFromUI() || '').trim(),
      };
  ...
  const saveAnnotation = useCallback(async (options = {}) => {
    ...
    // 单路 embedded：保存后继续编辑同一帧；勿走「上传视频标下一货架」的重置流程
    if (result.ok && !keepState && !embedded) {
      resetForNextShelf();
    }
  ...
  const loadAnnotationOnStartupRef = useRef(loadAnnotationOnStartup);
  loadAnnotationOnStartupRef.current = loadAnnotationOnStartup;
  const loadCameraSavedFrameRef = useRef(loadCameraSavedFrame);
  loadCameraSavedFrameRef.current = loadCameraSavedFrame;
  const initCanvasPlaceholderRef = useRef(initCanvasPlaceholder);
  initCanvasPlaceholderRef.current = initCanvasPlaceholder;

  useEffect(() => {
    if (!embedded || !fixedCamera?.id || streamOverlay) return undefined;
    const camId = String(fixedCamera.id);
    const loadGen = (embeddedLoadGenRef.current += 1);
    let cancelled = false;

    embeddedThumbSrcRef.current = '';
    bgImageRef.current = new Image();
    initCanvasPlaceholderRef.current();

    (async () => {
      const annOk = await loadAnnotationOnStartupRef.current({ silent: true });
      if (cancelled || loadGen !== embeddedLoadGenRef.current) return;
      const frameOk = await loadCameraSavedFrameRef.current({ silent: true });
      if (cancelled || loadGen !== embeddedLoadGenRef.current) return;
      if (annOk && frameOk) {
        setStatus('已加载已保存的标注与抓帧；更新画面请点「抓帧」。', 'ok');
      } else if (frameOk) {
        setStatus('已加载上次抓帧，可继续标注；完成后请保存。', 'ok');
      } else if (annOk) {
        setStatus('已加载标注；若画面空白请点「抓帧」绑定当前画面。', 'ok');
      } else {
        setStatus('请点「抓帧」获取画面后开始标注。', 'ok');
      }
    })();
  ...
  useEffect(() => {
    ...
    const onDown = (e) => {
      ...
      if (shelfPoints.length < 4) {
        shelfPointsRef.current = [...shelfPoints, [x, y]];
        if (shelfPointsRef.current.length === 4) {
          refreshPerspectiveFromShelf();
          setStatus('货架四角已齐，可拖动顶点微调，然后「生成货位」。', 'ok');
          setCanSave(!!mInvRef.current);
        } else {
          setStatus(
            `请继续点击货架角点（${shelfPointsRef.current.length}/4：左上→右上→右下→左下）`,
            'ok',
          );
        }
        renderAnnotator();
