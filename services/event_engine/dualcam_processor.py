...
class DualcamProcessor:
    ...
    def __init__(self, aisle: dict[str, Any]):
        self.aisle = aisle
        ...
        self.contact_m = float(aisle.get("contact_m", DEFAULT_CONTACT_M) or 0.0)
    ...
    def ready(self) -> bool:
        return bool(self.solved.get("ok") and "L" in self.cams and "R" in self.cams)
    ...
    def process_pair(self, pose_l: dict, pose_r: dict) -> dict[str, Any]:
        ...
        empty = {
            ...
            "collisions": [],
            "alarm_collisions": [],
            "skeletons": pose_l.get("persons") or [],
        }
        if not self.ready():
            return empty

        fl = keypoints_to_ks(pose_l.get("persons") or [])
        fr = keypoints_to_ks(pose_r.get("persons") or [])
        pairs = pick_pairs(fl, fr, self.cams, prefer=self._prefer)
        tokens: list[str] = []
        ...
        for i, j, _gap in pairs:
            ...
            for wi, idx in (("lw", LWRIST), ("rw", RWRIST)):
                ...
                for hit in hits:
                    tok = box_collision_token({
                        "shelf_code": self.shelf_code,
                        "box_id": hit.get("box_id") or "",
                    })
