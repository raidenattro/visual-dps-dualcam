"""Event Worker：双路 3D contact_slots，按 aisle_id 分片。"""
...
async def _run():
    ...
    if delivery == "stream":
        print(
            f"ℹ️ Event worker 已启动 delivery=stream dualcam-3d "
        ...
        print(
            f"ℹ️ Event worker 已启动 delivery=pubsub dualcam-3d ({shard_label()}) "
