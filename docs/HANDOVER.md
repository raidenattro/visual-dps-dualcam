...
更新时间：2026-08-27　分支：`exp/dualcam-play-perf`（自 `exp/dualcam-1-3`；基线成果在 `exp/tagged-aug85-v1`；门控在 `exp/action-gate-perf`）
...
## 双路三角化与墙面货格（2026-08-26/27，验证，未进推理）

1-3 组巷道两端对打拼接片，两路标同一拣货面，各自反解后 Umeyama 对到 A 的巷道系，再三角化 17 点。  
8.27：墙上均匀切货格 + `/play` 腕点贴墙变红。详见 `docs/daily/DAILY-2026-08-26.md`、`docs/daily/DAILY-2026-08-27.md`。

**货格复原只能当参考，不完全准确**：按墙矩形均匀 4×4，不是现场隔板；`align_rms=0` 不证明同一物理角。不要当线上货位真值。
...
1. **双路验收** — `/play` 对照伸进 vs 路过（贴墙变红）；只看有没有贴墙，**不要**用格号当准不准。残差大再微调四角重解（不必重跑姿态）
