...
- Worker：`visual-dps-event-worker` → `event_worker.py` → `DualcamRedisWorker`，分片键 `aisle_id`。
- 已移除 pick_state / `visual-dps-event-worker-2`。水平扩展用同一 3D 镜像的 `visual-dps-event-worker-b`（profile `worker-dual`）。
